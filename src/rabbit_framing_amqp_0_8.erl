...
-type(amqp_method_field_name() ::
       ( delivery_tag | ticket | queue | passive
       | durable | exclusive | global | auto_delete
       | identifier | nowait | integer_2 | prefetch_size
       | arguments | redelivered | ticket | no_local
       | queue | integer_result | message_count | no_ack
       | consumer_tag | consumer_count | delivery_tag | multiple
       | mandatory | type | nowait | immediate
       | ticket | reply_code | queue | exchange
       | routing_key | no_local | nowait | delivery_tag
       | arguments | delivery_tag | identifier | consumer_tag
       | reply_code | global | ticket | multiple
       | durable | queue | nowait | exchange
       | mandatory | integer_3 | immediate | message_count
       | delivery_tag | ticket | ticket | reply_code
       | frame_max | ticket | result | redelivered
       | queue | if_unused | ticket | if_empty
       | nowait | queue | heartbeat | exchange
       | delivery_tag | string_result | message_count | exchange
       | string_1 | routing_key | immediate | ticket
       | routing_key | consumer_tag | queue | queue
       | exchange | ticket | routing_key | queue
       | arguments | reply_text | no_ack | reply_text
       | exclusive | requeue | arguments | delivery_tag
       | redelivered | integer_4 | exchange | routing_key
       | consumer_tag | message_count | prefetch_size | version_minor
       | no_ack | prefetch_count | consumer_tag | global
       | known_hosts | prefetch_count | exchange | reply_code
       | exclusive | queue | operation | reply_text
       | exclusive | exchange | class_id | method_id
       | ticket | content_size | ticket | passive
       | out_of_band | routing_key | nowait | result
       | consumer_tag | no_local | exclusive | active
       | nowait | queue | active | virtual_host
       | active | string_2 | reply_code | exchange
       | identifier | reply_text | content_checksum | details
       | consumer_tag | reply_code | exchange | reply_text
       | reply_text | capabilities | exchange | class_id
       | requeue | nowait | consumer_tag | method_id
       | staged_size | integer_1 | meta_data | cluster_id
       | dtx_identifier | consumer_tag | version_major | consumer_tag
       | nowait | realm | read | requeue
       | server_properties | consumer_tag | mechanisms | operation
       | locales | write | client_properties | routing_key
       | mechanism | prefetch_count | response | routing_key
       | locale | mandatory | routing_key | challenge
       | routing_key | exchange | exchange | table
       | response | ticket | passive | channel_max
       | auto_delete | nowait | internal | delivery_tag
       | consumer_tag | nowait | prefetch_size | channel_max
       | consumer_tag | frame_max | consumer_tag | heartbeat
       | ticket | routing_key | exchange | requeue
       | if_unused | nowait | host | insist
       | consumer_tag | consume_rate | known_hosts | integer_op
       | ticket | string_op )).
