...
-type(amqp_method_field_name() ::
       ( immediate | version_major | version_minor | server_properties
       | mechanisms | locales | consumer_count | client_properties
       | mechanism | reply_code | response | locale
       | requeue | arguments | delivery_tag | challenge
       | response | credit | channel_max | frame_max
       | ticket | nowait | heartbeat | available
       | channel_max | frame_max | heartbeat | credit_drained
       | write | virtual_host | capabilities | insist
       | known_hosts | ticket | if_unused | queue
       | reply_code | reply_text | multiple | class_id
       | method_id | queue | message_count | reason
       | consumer_tag | out_of_band | routing_key | channel_id
       | prefetch_count | active | requeue | routing_key
       | active | consumer_tag | global | reply_code
       | reply_text | class_id | consumer_tag | method_id
       | consumer_tag | nowait | cluster_id | realm
       | exchange | exclusive | passive | active
       | delivery_tag | ticket | message_count | ticket
       | read | exchange | ticket | nowait
       | arguments | redelivered | ticket | exchange
       | type | exchange | passive | durable
       | consumer_tag | auto_delete | consumer_tag | delivery_tag
       | internal | exchange | nowait | arguments
       | message_count | ticket | ticket | exchange
       | ticket | if_unused | nowait | if_empty
       | routing_key | nowait | nowait | ticket
       | ticket | queue | destination | source
       | routing_key | multiple | reply_text | nowait
       | queue | arguments | queue | routing_key
       | exchange | queue | ticket | destination
       | requeue | source | routing_key | nowait
       | nowait | delivery_tag | arguments | routing_key
       | redelivered | exchange | drain | routing_key
       | requeue | prefetch_size | ticket | arguments
       | queue | no_ack | passive | durable
       | exclusive | auto_delete | nowait | mandatory
       | exclusive | arguments | no_ack | delivery_tag
       | consumer_tag | no_local | queue | message_count )).
